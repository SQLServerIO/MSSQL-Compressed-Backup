...
{
    ///-------------------------------------------------------------------------------------------------
    /// <summary>	QuickLZ stream. </summary>
    ///
    /// <remarks>	Wes Brown, 5/26/2009. </remarks>
    ///-------------------------------------------------------------------------------------------------

    public class QuickLZStream : Stream
    {
        #region DLLImports
        ...
        {
            get { return (uint) qlz_get_setting(0); }
        }
*/

        /*
                public uint QlzScratchCompress
                {
                    get { return (uint) qlz_get_setting(1); }
                }
        */

        /*
                public uint QlzScratchDecompress
                {
                    get { return (uint) qlz_get_setting(2); }
                }
        */

        /*
                public uint QlzVersionMajor
                {
                    get { return (uint) qlz_get_setting(7); }
                }
        */

        /*
                public uint QlzVersionMinor
                {
                    get { return (uint) qlz_get_setting(8); }
                }
        */

        /*
                        public int QlzVersionRevision
                        {
                            // negative means beta
                            get { return qlz_get_setting(9); }
                        }
                */

        [DllImport(@"QuickLZ\QuickLZ.dll")]
        // ReSharper disable InconsistentNaming
        private static extern unsafe uint* qlz_compress(
        ...
        [DllImport(@"QuickLZ\QuickLZ.dll")]
        // ReSharper disable InconsistentNaming
        private static extern unsafe uint* qlz_decompress(
            // ReSharper restore InconsistentNaming
            byte* source,
            byte* destination,
            byte* scratch);
        ...
        [DllImport(@"QuickLZ\QuickLZ.dll")]
        // ReSharper disable InconsistentNaming
        private static extern unsafe uint* qlz_size_compressed(byte* source);
        // ReSharper restore InconsistentNaming

        [DllImport(@"QuickLZ\QuickLZ.dll")]
        // ReSharper disable InconsistentNaming
        private static extern unsafe IntPtr qlz_size_decompressed(byte* source);
        // ReSharper restore InconsistentNaming
        ...
        public unsafe uint SizeCompressed(byte* source)
        ...
        // ReSharper disable UnusedMember.Global
        public unsafe uint SizeDecompressed(byte* source)
        // ReSharper restore UnusedMember.Global
        {
        ...
        private static int _fifosize = 4096 * 1024;
        ...
        */
        ...
        //used by QuickLZ to do its work on compress
        //lvl1_32bit 36880 lvl2_32bit 34832 lvl3_32bit 266256 lvl1_64bit 69648 lvl2_64bit 67600  lvl3_32bit 528400
        private readonly byte[] _scratchCompress = new byte[36880];

        //used by QuickLZ to do its work on decompress
        //lvl1_32bit 20496 lvl2_32bit 34832 lvl3_32bit 16 lvl1_64bit 36880 lvl2_64bit 67600  lvl3_32bit 16
        private readonly byte[] _scratchDecompress = new byte[20496];
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>	Gets the qlz streaming buffer. </summary>
        ///
        /// <value>	A Buffer for qlz streaming data. </value>
        ///-------------------------------------------------------------------------------------------------
        private static uint QlzStreamingBuffer
        {
            get { return (uint)qlz_get_setting(3); }
        }

        /*
                public bool QlzMemorySafe
                {
                    get { return qlz_get_setting(6) == 1 ? true : false; }
                }
        */
        ...
        ///-------------------------------------------------------------------------------------------------
        public QuickLZStream(Stream stream, CompressionMode mode)
        ...
        ///-------------------------------------------------------------------------------------------------
        private QuickLZStream(Stream stream, CompressionMode mode, bool leaveOpen)
        {
            _recordsread = 0;
            if (stream == null)
...
//            Console.WriteLine("EqueueReadData:   " + st.ElapsedTicks);
        ...
        private unsafe void EqueueReadData()
        {
                   ...
                   (fb >= _buffermap[_buffermappointer]))
            {
                ...
                fixed (byte* src = _compressed, dst = _decompressed, sdec = _scratchDecompress)
                {
                    //try to read the incomming _stream
                    try
                    {
                        //_bytes = 
                            BaseStream.Read(_compressed, 0, PacketMap[_packetmappointer]);
                    }
                    catch
                    {
                        //we had a failure 
...
#if DEBUG
                    Console.WriteLine("Bytes Read:                " + _bytes); //debug
#endif
                    //doing this check during the debug process
                    //specific to QuickLZ this gets the _bytes decompressed from the
                    //archive header
                    var dsize = (uint)qlz_size_decompressed(src);

                    //should be safe to remove from release version
                    if (dsize > fb)
                    {
                        Console.WriteLine("Decompressed block bigger than space available");
                        break;
                    }
#if DEBUG
                    Console.WriteLine("Expected Decompressed Size: " + dsize); //debug
#endif
                    //do the decompression
                    _dstlen = (int)qlz_decompress(src, dst, sdec);
#if DEBUG
                    Console.WriteLine("Returned Decompressed Size: " + _dstlen); //debug
#endif
...
//            Console.WriteLine("------------------------------------------------");
            ...
            _compressed = new byte[buffsize + 400];
...
//            Console.WriteLine("_bibreadBuffer.Get:      " + st.ElapsedTicks);
            ...
            fixed (byte* source = _compressbuff, destination = _compressed, scratch = _scratchCompress)
            {
                //call the QuickLZ compressor
                st.Start();
                _dstlen = (int)qlz_compress(source, destination, buffsize, scratch);
                st.Stop();
                _timecomressing += st.ElapsedTicks;
//                Console.WriteLine("qlz_compress:            " + st.ElapsedTicks);
                st.Reset();
                st.Start();
