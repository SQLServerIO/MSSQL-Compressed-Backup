...
{
    public class BriefLZStream : Stream
    {
        ///-------------------------------------------------------------------------------------------------
        /// <summary>   
        /// Blz pack.
        ...
        /// </summary>
        /// <remarks>	Wes Brown, 6/5/2009. </remarks>
        ///
        /// <param name="source">		pointer to the data to be compressed. </param>
        /// <param name="destination">	pointer to where the compressed data should be stored. </param>
        /// <param name="length">		the length of the uncompressed data in bytes. </param>
        /// <param name="workmem">		pointer to the work memory which is used during compression. </param>
        ///
        /// <returns>the length of the compressed data, or -1 on error. </returns>
        ///-------------------------------------------------------------------------------------------------
        [DllImport(@"Unmanaged Resources\brieflz.dll")]

        private static extern unsafe uint blz_pack(
            byte* source,
            byte* destination,
            int length,
            byte* workmem);
        ...
        /// <remarks>	Wes Brown, 6/5/2009. </remarks>
        ///
        /// <param name="source">			pointer to the compressed data. </param>
        /// <param name="destination">		pointer to where the decompressed data should be stored. </param>
        /// <param name="depacked_length">	the size in bytes of the decompressed data. </param>
        ///
        /// <returns> the length of the decompressed data, or -1 on error.</returns>
        ///-------------------------------------------------------------------------------------------------
        [DllImport(@"Unmanaged Resources\brieflz.dll")]
        private static extern unsafe uint blz_depack(
			byte* source,
			byte* destination,
            int depacked_length
		);
		...
		/// <param name="length">	the length of the uncompressed data in bytes. </param>
		///
        /// <returns>the required size of the work buffer.</returns>
		///-------------------------------------------------------------------------------------------------
        [DllImport(@"Unmanaged Resources\brieflz.dll")]
		public static extern int blz_workmem_size(uint length);

		///-------------------------------------------------------------------------------------------------
		/// <summary>	
		/// Blz maximum packed size. 
		...
		///-------------------------------------------------------------------------------------------------
        [DllImport(@"Unmanaged Resources\brieflz.dll")]
		public static extern int blz_max_packed_size(uint length);

        ///-------------------------------------------------------------------------------------------------
        /// <summary>	
        /// Blz crc 32. 
        /// Computes the CRC32 value of length bytes of data from source[]. 
        /// initial_crc32 is used as the starting crc value, allowing the function to be used to 
        /// checksum multiple blocks of memory as if they were one large block. 
        /// Pass zero on the first call, and the current result on the following calls.
        /// </summary>
        ///
        /// <remarks>	Wes Brown, 6/5/2009. </remarks>
        ///
        /// <param name="source">			pointer to the data to process. </param>
        /// <param name="length">			the size in bytes of the data. </param>
        /// <param name="initial_crc32">	the current CRC32 value (pass 0 for first block). </param>
        ///
        /// <returns>	the CRC32 value. </returns>
        ///-------------------------------------------------------------------------------------------------
        [DllImport(@"Unmanaged Resources\brieflz.dll")]
        public static extern unsafe uint blz_crc32(
            byte* source,
            int length,
            uint initial_crc32);
        ...
        */
        ...
        ///-------------------------------------------------------------------------------------------------
        public BriefLZStream(Stream stream, CompressionMode mode)
        ...
        ///-------------------------------------------------------------------------------------------------
        public BriefLZStream(Stream stream, CompressionMode mode, bool leaveOpen)
        {
            _recordsread = 0;
            if (stream == null)
...
//            Console.WriteLine("EqueueReadData:   " + st.ElapsedTicks);
        ...
        private unsafe void EqueueReadData()
        {
                   ...
                   (fb >= _buffermap[_buffermappointer]))
            {
                ...
                fixed (byte* src = _compressed, dst = _decompressed, sdec = _scratchCompress)
                {
                    ...
                    _dstlen = (int)blz_depack(src, dst, _buffermap[_buffermappointer]);
#if DEBUG
...
//            Console.WriteLine("------------------------------------------------");
            ...
            _compressed = new byte[blz_max_packed_size((uint) buffsize)];
...
//                Console.WriteLine("blz_pack:            " + st.ElapsedTicks);
                st.Reset();
                st.Start();
