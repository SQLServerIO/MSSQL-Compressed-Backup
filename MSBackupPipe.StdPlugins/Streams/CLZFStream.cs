...
{
    public class CLZFStream : Stream
    {
        readonly CLZF _lzf = new CLZF();
        ...
        */
        ...
        ///-------------------------------------------------------------------------------------------------
        public CLZFStream(Stream stream, CompressionMode mode)
        ...
        ///-------------------------------------------------------------------------------------------------
        public CLZFStream(Stream stream, CompressionMode mode, bool leaveOpen)
        {
            _recordsread = 0;
            if (stream == null)
        ...
        private unsafe void EqueueReadData()
        {
                   ...
                   (fb >= _buffermap[_buffermappointer]))
            {
                ...
                _dstlen = _lzf.lzf_decompress(_compressed, _fifosize, _decompressed, 0);
    ...
    //how much space have we consumed?
        ...
        private unsafe void DequeueWriteData()
        {
            ...
            st.Start();
            _dstlen = _lzf.lzf_compress(_compressbuff,_fifosize,_compressed,_fifosize);
            ...
            Console.WriteLine("lzf_compress:            " + st.ElapsedTicks);
            st.Reset();
            st.Start();
