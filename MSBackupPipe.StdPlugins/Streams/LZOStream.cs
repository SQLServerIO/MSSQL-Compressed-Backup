...
namespace PlanetaryDBLib.IO.Compression
{
    ...
    public class LzoStream : Stream
    {
        ...
        [DllImport(@"Unmanaged Resources\lzo.dll")]
        private static unsafe extern int lzo1x_1_compress(
            byte* src,
            int src_len,
            byte* dst,
            int* dst_len,
        ...
        [DllImport(@"Unmanaged Resources\lzo.dll")]
        private static unsafe extern int lzo1x_decompress(
            byte* src,
            int src_len,
            byte* dst,
            int* dst_len,
        ...
        //set at twice the size so if you try and compress something that won't
        //compress and is larger than the input buffer it won't blow up the output buffer.
        private byte[] dest = new byte[1048576 * 4];
        ...
        */
        ...
        ///-------------------------------------------------------------------------------------------------
        private LzoStream(Stream stream, CompressionMode mode, bool leaveOpen)
        {
            _recordsread = 0;
            if (stream == null)
...
//            Console.WriteLine("EqueueReadData:   " + st.ElapsedTicks);
        ...
        private unsafe void EqueueReadData()
        {
                   ...
                   (fb >= _buffermap[_buffermappointer]))
            {
                ...
                fixed (byte* src = _compressed, dst = _decompressed, sdec = _workMemory)
                {
...
#endif
                    //specific to LzoStream
                    //get pointer to _destlen
                    int* pdstlen = &destlen;
                    //do the decompression
                    lzo1x_decompress(dst, PacketMap[_packetmappointer], src, pdstlen, sdec);
#if DEBUG
                    Console.WriteLine("Returned Decompressed Size: " + _dstlen); //debug
#endif

                    //enqueue our decompressed block
                    _bibreadBuffer.Put(ref _decompressed, ref _decompressedoffset, ref destlen, out _reccount);
#if DEBUG
...
//            Console.WriteLine("_bibreadBuffer.Get:      " + st.ElapsedTicks);
            ...
            fixed (byte* source = _compressbuff, destination = _compressed, scratch = _workMemory)
            {
                ...
                lzo1x_1_compress(source, reccnt ,destination, dst_len, scratch);
                st.Stop();
                _timecomressing += st.ElapsedTicks;
//                Console.WriteLine("lzo_compress:            " + st.ElapsedTicks);
                st.Reset();
                st.Start();
