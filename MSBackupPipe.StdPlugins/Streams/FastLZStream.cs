...
///-------------------------------------------------------------------------------------------------
//	namespace:	PlanetaryDB.IO.Compression
//
//	summary:	.
//
///-------------------------------------------------------------------------------------------------
namespace PlanetaryDBLib.IO.Compression.Streams
{
    ...
    public class FastLZStream : Stream
    {
        ...
        [DllImport(@"Unmanaged Resources\FastLZ.dll")]
        private static extern unsafe int fastlz_compress(byte* source, int length, byte* destination);

        [DllImport(@"Unmanaged Resources\FastLZ.dll")]
        private static extern unsafe int fastlz_decompress(byte* source, byte* destination, int maxout);

        [DllImport(@"Unmanaged Resources\FastLZ.dll")]
        private static extern unsafe int fastlz_compress_level(int level, byte* source, int length, byte* destination);
        ...
        ///-------------------------------------------------------------------------------------------------
        public FastLZStream(Stream stream, CompressionMode mode)
        ...
        ///-------------------------------------------------------------------------------------------------
        private FastLZStream(Stream stream, CompressionMode mode, bool leaveOpen)
        {
            _recordsread = 0;
            if (stream == null)
        ...
        private unsafe void EqueueReadData()
        {
                   ...
                   (fb >= _buffermap[_buffermappointer]))
            {
                ...
                fixed (byte* src = _compressed, dst = _decompressed)
                {
                    //try to read the incomming _stream
                    try
                    {
                        _bytes = BaseStream.Read(_compressed, 0, PacketMap[_packetmappointer]);
                    }
                    catch
                    {
                        //we had a failure 
                    ...
                    _dstlen = fastlz_decompress(src, dst,_compressed.Length);
#if DEBUG
        ...
        private unsafe void DequeueWriteData()
        {
            ...
            fixed (byte* source = _compressbuff, destination = _compressed)
            {
                //call the QuickLZ compressor
                st.Start();
                _dstlen = fastlz_compress_level(2,source, buffsize, destination);
                st.Stop();
